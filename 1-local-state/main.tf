...
output "test_cURL" {
  value = <<EOF
curl -X POST -H 'Content-Type: application/json' -d '{"id":"test", "docs":[{"key":"value"}]}' ${aws_api_gateway_stage.main.invoke_url}/sqs1
curl -X POST -H 'Content-Type: application/json' -d '{"id":"test", "docs":[{"key":"value"}]}' ${aws_api_gateway_stage.main.invoke_url}/sqs2
EOF
}
...
resource "aws_api_gateway_integration" "api2" {
  ...
  depends_on = [aws_api_gateway_rest_api.api, aws_api_gateway_resource.api2] 
}
